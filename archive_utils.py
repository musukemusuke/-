...
def create_chat_archive(messages, channel_name):
    ...
    try:
        ...
        print("archive_utils: MSゴシックフォントを使用")
    except Exception as e:
        print(f"archive_utils: MSゴシック登録失敗: {e}")
        try:
            ...
            if font_path:
                pdfmetrics.registerFont(TTFont('jp_font', font_path))
                print(f"archive_utils: Noto Sansフォントを使用 (path={font_path})")
            else:
                raise Exception("フォントがどのパスにも存在しません")
        except Exception as e:
            print(f"archive_utils: フォント登録失敗: {e}")
            # フォント一覧を出力
    ...
    c.save()
    buffer.seek(0)
    print(f"archive_utils: PDF生成完了、チャンネル名={channel_name}, メッセージ数={len(messages)}, ページ数={page_num}")
    
    # PDFを画像に変換
    try:
        print("archive_utils: PDFを画像に変換開始")
        ...
        img_buffer.seek(0)
        print("archive_utils: PDFから画像への変換完了")
        # PDFバッファを巻き戻して返却
        buffer.seek(0)
        return buffer, img_buffer
    except Exception as e:
        print(f"archive_utils: PDFから画像への変換に失敗: {e}")
        # 画像変換に失敗してもPDFは返却する
        buffer.seek(0)
        return buffer, None